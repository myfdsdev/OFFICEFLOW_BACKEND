...
export default function DirectMessagesList({ currentUser, onUserSelect }) {
  ...
  // Fetch users (extracted for re-use in subscriptions)
  const fetchUsers = async () => {
    ...
    try {
      ...
      const normalized = userList.map((u) => ({ ...u, id: u.id || u._id }));
      setUsers(normalized);
      await fetchUnreadCounts(normalized);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    // Real-time: refresh unread counts when new message arrives
    const messageUnsubscribe = base44.entities.Message.subscribe((event) => {
      if (event.type === "create" && event.data) {
        const msg = event.data;
        // Only refresh if message is to current user
        if (String(msg.receiver_id) === String(currentUser.id)) {
          fetchUnreadCounts(users.length ? users : []);
          // Also refetch user list to update online status timestamps
          fetchUsers();
        }
      }
    });

    // Real-time: update online/offline status live
    const userUnsubscribe = base44.entities.User.subscribe((event) => {
      if (event.type === "update" && event.data) {
        const updatedId = String(event.data._id || event.data.id);
        setUsers((prev) =>
          prev.map((u) =>
            String(u.id) === updatedId
              ? { ...u, is_online: event.data.is_online, last_active_time: event.data.last_active }
              : u
          )
        );
      } else if (event.type === "create") {
        // New user joined — refresh full list
        fetchUsers();
      }
    });
  ...
  const getLastSeenText = (user) => {
    ...
    if (user.last_active_time || user.last_active) {
      try {
        return `Last seen ${formatDistanceToNow(
          new Date(user.last_active_time || user.last_active),
          { addSuffix: true }
        )}`;
      } catch (e) {
        return "Offline";
