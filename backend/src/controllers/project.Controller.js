...
export const filterProjects = asyncHandler(async (req, res) => {
  ...
  if (req.user.role !== "admin") {
    ...
    // If filter already has _id, make sure that _id is in member list
    if (filter._id) {
      const requestedId = filter._id.toString();
      if (!memberProjectIds.includes(requestedId)) {
        return res.json([]); // not a member of requested project
      }
      // Keep the specific _id filter
    } else {
      // No specific _id requested → show all member projects
...
export const updateProject = asyncHandler(async (req, res) => {
  ...
  // Permission: owner, admin member, or system admin
  ...
  // Sync project_name on related docs if changed
